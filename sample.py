...
def sample(pair, dataloaders, exp_args, args):
    ...
    # 确定train和test的样本数量
    n_train_samples = round(train_pct * args.n)   # args.n is the number of all samples
    n_test_samples = args.n - n_train_samples

    def sample_split(split, n):
        stats = util.Statistics()   # Statistics 是一个自建的类，存放了一些统计学的方法
        all_lang = pd.DataFrame()   # 创建一个数据库准备存所有language
        pbar = tqdm(desc=f"sample {split}", total=n)    # 创建进度条
        # pdb.set_trace()

        print(all_lang.shape[0])
        while all_lang.shape[0] < n:
            split_stats, lang = run(
                split, 0, pair, None, dataloaders, exp_args, force_no_train=True
            )   # split代表运行模式，为train或者test，0代表当前epoch，  # pair指当前处理的speaker, listener对
