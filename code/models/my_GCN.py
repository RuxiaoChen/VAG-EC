...
# import prepare_listener_data_test as ld
from torch_geometric.nn import SAGEConv, to_hetero, GCNConv, GINConv
...
class GlobalPoolingGraphNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(GlobalPoolingGraphNN, self).__init__()
        self.conv1 = torch.nn.Linear(input_dim, hidden_dim)
        self.conv2 = torch.nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        x = torch.relu(self.conv1(x))  # 应用第一个全连接层并使用ReLU激活函数
        x = self.conv2(x)  # 应用第二个全连接层，不使用激活函数
        x = torch.mean(x, dim=0, keepdim=True)  # 对所有节点的输出取平均，得到全局特征
        return x
...
class GIN(nn.Module):
    def __init__(self, num_features, hidden_dim, num_classes):
        ...
        self.conv1 = GINConv(nn.Linear(num_features, hidden_dim))
        self.conv2 = GINConv(nn.Linear(hidden_dim, hidden_dim))
        self.conv3 = GINConv(nn.Linear(hidden_dim, num_classes))

        # Define readout function (global pooling)
        self.pooling = nn.GlobalMaxPool()
    ...
    def forward(self, x, edge_index):
        ...
        # # Global pooling to obtain a single graph-level representation
...
class GINModel(nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        self.downconv1 = make_convolution(num_features, 1024)
        self.downconv2 = make_convolution(1024, 1024)
        self.upconv1 = make_convolution(1024 + num_features, 1024)
        self.upconv2 = make_convolution(1024 + 1024, 1024)
