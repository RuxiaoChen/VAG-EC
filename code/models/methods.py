...
def normalized_laplacian(adjacency_matrix: torch.Tensor) -> torch.Tensor:
    ...
    Ln = torch.eye(num_nodes, device=adjacency_matrix.device) - torch.mm(torch.mm(Dinv_sqrt, adjacency_matrix),
                                                                         Dinv_sqrt)
    Ln = 0.5 * (Ln + Ln.T)
    return Ln
...
def all_centralities(graph):
    ...
    centralities = [torch.tensor(extract_numerical_values(centralities_dict[method_name]), dtype=torch.float) for
                    (_, method_name) in centrality_methods]
...
def top_k_pool(scores, edge_index, h, ratio):
    ...
    un_g = torch.matmul(g.bool().float(),
                        torch.matmul(g.bool().float(), g.bool().float())).bool().float()  # Calculate unnormalized graph
