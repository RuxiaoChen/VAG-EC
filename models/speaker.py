...
class CopySpeaker(nn.Module):
    ...
    def embed_features(self, feats, edges, model_gcn, attention_s, targets=None):
        """
        ...
        for i in range(len(parts_flat)):
            ...
            gra_fea = model_gcn(node_fea, all_edges[i].long().cuda())
            ## Global Mean Pooling
            # global_mean_pool = torch.mean(gra_fea, dim=0)
            # all_graph_feat.append(global_mean_pool)

            ## Global Attention Pooling
            attention_weights = attention_s(gra_fea)
            # 应用注意力权重
...
class Speaker(CopySpeaker):
    def __init__(
        ...
        self.my_init_h = nn.Linear(self.feat_size, self.hidden_size)
    ...
    def forward(self, feats, targets, inp_parts, all_edges, model_gcn, attention_s, **kwargs):
        """Sample from image features"""
        feats_emb = self.embed_features(inp_parts, all_edges, model_gcn, attention_s, targets)
        # feats_emb = self.embed_features(feats, all_edges, model_gcn, targets)
